<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            {{-- Tombol Back akan mengembalikan ke Index dengan tanggal yang sama --}}
            <a href="{{ route('guru.riwayat-absensi.index') }}?tanggal={{ $tanggalFilter }}" class="p-2 bg-white rounded-full shadow-sm border border-gray-200 hover:bg-gray-50 transition-colors">
    ...
    <div class="py-12 bg-gray-50 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            {{-- HEADER INFO --}}
            <div class="bg-white shadow-sm rounded-[2rem] border border-gray-100 p-8 flex flex-col md:flex-row justify-between items-center gap-4">
                <div>
                ...
                <div class="bg-gray-50 border border-gray-200 px-6 py-3 rounded-2xl text-center shadow-inner">
                    <span class="block text-xs text-gray-500 font-bold uppercase tracking-widest mb-1">Tanggal Terpilih</span>
                    <span class="block text-lg font-black text-orange-600">{{ \Carbon\Carbon::parse($tanggalFilter)->isoFormat('DD MMMM YYYY') }}</span>
                </div>
            </div>

            {{-- FORM TABEL SISWA --}}
            <form action="{{ route('guru.riwayat-absensi.update', $jadwal->id) }}" method="POST">
                @csrf
                ...
                <div class="bg-white shadow-sm rounded-[2rem] border border-gray-100 p-8">
                    <div class="mb-6 flex justify-between items-center border-b border-gray-100 pb-4">
                        <h3 class="text-lg font-black text-gray-800 uppercase tracking-wide">Daftar Kehadiran</h3>
                        ...
                        <table class="w-full text-left">
                            <thead>
                                <tr class="text-gray-400 text-xs uppercase tracking-widest border-b border-gray-100">
                                    ...
                                    <th class="pb-4 pr-4 font-black text-right w-48">Status</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-50">
                                ...
                                <tr class="group hover:bg-gray-50/50 transition-colors">
                                    <td class="py-4 pl-4 font-medium text-gray-500">{{ $index + 1 }}</td>
                                    <td class="py-4 font-bold text-gray-700">{{ $rs->siswa->nis }}</td>
                                    <td class="py-4 font-bold text-gray-900">{{ $rs->siswa->nama }}</td>
                                ...
                                @empty
                                <tr>
                                    <td colspan="5" class="py-12 text-center text-gray-400 italic">Belum ada siswa di kelas ini.</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
