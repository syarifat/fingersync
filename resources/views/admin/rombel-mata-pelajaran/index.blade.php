<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-2xl text-gray-800 leading-tight italic">
            {{ __('Plotting Guru Mata Pelajaran') }}
        </h2>
    </x-slot>
            ...
            <div class="bg-white shadow-sm rounded-[2rem] border border-gray-100 overflow-hidden">
                <div class="p-8">
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                        <div>
                            <h3 class="text-lg font-bold text-orange-600 uppercase tracking-tighter">Pembagian Tugas Mengajar</h3>
                            <p class="text-sm text-gray-500">
                                Tahun Ajar: <span class="font-bold text-black">{{ session('tahun_ajar_nama', '-') }}</span>
                            </p>
                        </div>
                        <a href="{{ route('admin.rombel-mata-pelajaran.create') }}" class="px-6 py-3 bg-orange-600 text-white rounded-2xl font-bold text-xs uppercase tracking-widest hover:bg-orange-700 transition-all shadow-lg shadow-orange-100 flex items-center gap-2">
                    ...
                    <div class="mb-6 bg-gray-50 p-5 rounded-2xl border border-gray-100">
                        <form method="GET" action="{{ route('admin.rombel-mata-pelajaran.index') }}" class="flex flex-col md:flex-row gap-4">
                            ...
                            <div class="md:w-1/4">
                                <label for="guru_id" class="block text-xs font-bold text-gray-500 uppercase mb-1">Filter Guru</label>
                                <select name="guru_id" id="guru_id" class="block w-full rounded-xl border-gray-200 bg-white text-sm focus:border-orange-500 focus:ring-orange-500 shadow-sm">
                            ...
                            <div class="flex items-end gap-2">
                                ...
                                @if(request()->hasAny(['search', 'kelas_id', 'guru_id']))
                                <a href="{{ route('admin.rombel-mata-pelajaran.index') }}" class="px-4 py-2.5 bg-white border border-gray-300 text-gray-700 text-sm font-bold rounded-xl hover:bg-gray-50 transition-colors flex items-center justify-center" title="Reset">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <table class="w-full text-left">
                            <thead>
                                <tr class="text-gray-400 text-xs uppercase tracking-widest border-b border-gray-100">
                                    <th class="pb-4 pl-4 font-black">Kelas</th>
                                    <th class="pb-4 font-black">Mata Pelajaran</th>
                                    <th class="pb-4 font-black">Guru Pengajar</th>
                                    <th class="pb-4 pr-4 font-black text-right">Aksi</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-50">
                                @forelse ($rombelMapel as $r)
                                <tr class="group hover:bg-orange-50/20 transition-colors">
                                    ...
                                    <td class="py-5">
                                        <div class="font-bold text-gray-800">{{ $r->mataPelajaran->nama ?? '-' }}</div>
                                    </td>
                                    <td class="py-5">
                                        <div class="flex items-center gap-3">
                                            @if($r->guru && $r->guru->image)
                                            <img src="{{ asset('img/guru/'.$r->guru->image) }}" class="w-8 h-8 rounded-full object-cover border border-gray-200">
                                            @else
                                            <div class="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-xs font-bold text-gray-500">
                                                {{ substr($r->guru->nama ?? '?', 0, 1) }}
                                            </div>
                                            @endif
                                            <div>
                                                <div class="text-sm font-bold text-gray-700">{{ $r->guru->nama ?? 'Belum diset' }}</div>
                                                <div class="text-[10px] text-gray-400 uppercase tracking-wider">NIDN: {{ $r->guru->nidn ?? '-' }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="py-5 pr-4 text-right">
                                        <div class="flex justify-end gap-3">
                                            <a href="{{ route('admin.rombel-mata-pelajaran.edit', $r->id) }}" class="p-2 bg-orange-100 text-orange-600 rounded-lg hover:bg-orange-600 hover:text-white transition-all shadow-sm" title="Edit">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                                </svg>
                                            </a>
                                            <form action="{{ route('admin.rombel-mata-pelajaran.destroy', $r->id) }}" method="POST" onsubmit="return confirm('Hapus plotting mapel ini?')">
                                                @csrf @method('DELETE')
                                ...
                                <tr>
                                    <td colspan="4" class="py-12 text-center text-gray-400 italic">
                                        Belum ada data plotting guru di tahun ini.
                                    </td>
                                </tr>
                                @endforelse
                            </tbody>
                    ...
                    <div class="mt-8">{{ $rombelMapel->links() }}</div>
                </div>
            </div>
        </div>
    </div>
