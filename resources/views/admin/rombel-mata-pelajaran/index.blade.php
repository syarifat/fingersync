            ...
            <div class="bg-white shadow-sm rounded-[2rem] border border-gray-100 p-8">
                ...
                <div class="mb-8 border-b border-gray-100 pb-6">
                    ...
                    <p class="text-sm text-gray-500 mt-1">
                        Pilih kelas di bawah ini untuk membagikan tugas mengajar (Mapel & Guru).
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div class="border border-gray-200 rounded-2xl p-6 hover:shadow-lg transition-all bg-white relative overflow-hidden group">
                        <div class="absolute top-0 left-0 w-full h-1 {{ $k->jumlah_mapel > 0 ? 'bg-orange-500' : 'bg-gray-300' }}"></div>
                        
                        <div class="flex justify-between items-center mb-6">
                            <h4 class="text-2xl font-black text-gray-800">{{ $k->nama }}</h4>
                            <span class="px-3 py-1 bg-orange-50 text-orange-600 rounded-full text-xs font-bold border border-orange-100">
                                {{ $k->jumlah_mapel }} Mapel
                            </span>
                        </div>

                        <a href="{{ route('admin.rombel-mata-pelajaran.manage', $k->id) }}" class="block w-full py-2.5 text-center bg-gray-800 text-white rounded-xl text-sm font-bold hover:bg-orange-600 transition-colors">
                            Kelola Plotting
                        </a>
                    </div>
                    @endforeach
