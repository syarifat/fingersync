...
void jalankanValidasi() {
  ...
  if (result == FINGERPRINT_OK) {
    ...
    // --- BYPASS MODE EXPO: Deteksi Jari Siapapun ---
    int sendId = 999; // ID 999 untuk Jari Pengunjung / Tamu Expo
    result = finger.image2Tz();
    if (result == FINGERPRINT_OK) {
      if (finger.fingerFastSearch() == FINGERPRINT_OK) {
        sendId = finger.fingerID;
        Serial.print("[VALIDASI] Jari Terdaftar di Sensor! ID: ");
        Serial.println(sendId);
      } else {
        Serial.println("[VALIDASI] Jari Tamu / Belum Terdaftar (Mode Expo: Diteruskan ke Server)");
      }
    }
    ...
    lcd.setCursor(0, 1);
    lcd.print("ID: "); 
    lcd.print(sendId == 999 ? "TAMU EXPO" : String(sendId));

    // Kirim langsung ke server agar respon diatur via Panel Web Expo
    kirimAbsensi(sendId);
